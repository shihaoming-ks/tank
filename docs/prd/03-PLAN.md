...
| Stage | 主题 | 产出 | 依赖外部资源 | 预估 |
|---|---|---|---|---|
| **S1** | 纯代码本地闭环 | `npm run dev` 双窗口打完一局 | ❌ 无 | 3.5h |
| **S2** | 健壮性与可测试性 | 异常处理 + 单测 + 结构化日志 | ❌ 无 | 1h |
...
| # | 内容 | 验证方式 |
|---|---|---|
| **S1.1** | 脚手架：`server/index.js` 托管 `client/`+`shared/`、`/healthz`、WS echo | 浏览器 Console 看到 echo 回包 |
| **S1.2** | 房间与玩家：`RoomManager`/`Room`、join/leave/断线/满员拒绝、大厅 UI | 两窗口互见玩家列表；关窗 3s 内更新 |
| **S1.3** ★ | 移动同步：`map.js`+`physics.js`+30Hz 广播+`render.js` | A 移动 B 立刻看到；撞墙停住；不越界 |
| **S1.4** | 战斗闭环：射击冷却、命中扣血、淘汰复活、胜负结算、HUD、结算面板 | 双端 HP 与胜负结果完全一致 |
...
| 子弹不伤发射者 | 发射者 HP 不变 |
| 无敌期不扣血 | HP 不变 |
| 存活 1 人 → 结束 | `reason === 'last_survivor'` |
| 时间到 → 比 HP | 高 HP 者为 winner；相同则 `winnerId === null` |
...
- [ ] `npm test` 全绿
- [ ] 空昵称被拦并有提示
- [ ] `kill` server 进程后客户端出现断开遮罩
- [ ] 服务端日志为单行 JSON，含 `roomId`/`playerId`
- [ ] 连续打 3 局均正常判定结束
...
| 风险 | 阶段 | 对策 |
|---|---|---|
| 坦克 24px 卡在 32px 网格通道 | S1 | 地图通道 ≥2 格宽；碰撞失败直接停，不做滑动修正 |
| 高频 `input` 刷爆上行 | S1 | 仅在按键状态**变化**时发送，非每帧发 |
| `setInterval` 漂移累积 | S1 | tick 内用真实 `dt` 积分，不假设固定 33ms |
| 两窗口共享 localStorage 串号 | S1 | 身份完全由服务端分配 `selfId`，客户端不持久化 |
...
**S1～S2（核心，必须全绿）**
- [ ] 干净环境 `npm install && npm run dev` 一次成功
- [ ] 两窗口同房间完成一局，双端 HP 与胜负结果一致
- [ ] 撞墙/越界/不自伤/无敌期规则正确
- [ ] 关窗后另一端 3s 内感知
- [ ] 无效输入有提示；服务端异常不崩进程
- [ ] `npm test` 全绿
