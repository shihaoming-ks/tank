...
| `npm run smoke` | 运行端到端冒烟测试（**需先启动服务**） |

### 冒烟测试

`npm run smoke` 用真实 WebSocket 客户端模拟多名玩家，
自动验证房间生命周期与全部边界情况（共 39 项断言）：
...
覆盖范围：创建/加入房间、颜色与槽位分配、权限校验、无效输入、
房间容量上限、主动离开、断线移除、房主移交、开局条件、房间回收。
